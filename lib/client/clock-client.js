...
client.render = function render () {
  ...
  // Restyle body bg
  if (thresholdReached) {
    $('body').css('background-color', 'grey').css('color', 'white').css('text-shadow', '2px 2px 4px rgba(0, 0, 0, 0.8)');
    $('.ar').css('filter', 'brightness(0%)').html(arrow);
  } else {
    $('body').css('color', bgColor ? 'white' : 'white').css('text-shadow', '2px 2px 4px rgba(0, 0, 0, 0.8)');
    $('.ar').css('filter', bgColor ? 'brightness(100%)' : 'brightness(100%)').html(arrow);
  }
