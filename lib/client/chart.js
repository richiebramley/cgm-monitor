...
function init (client, d3, $) {
  ...
  chart.update = function update (init) {
    ...
    var contextHeight = TODOHeight = chartHeight * .3;
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        TODO.select('.x.brush').select('.overlay').style('cursor', 'move');
        TODO.select('.x.brush').selectAll('.handle')
          .style('cursor', 'move');

        TODO.select('.x.brush').select('.selection')
  ...
  chart.updateContext = function(dataRange_) {
    ...
    TODO.select('.x').call(chart.xAxis2);
  };
